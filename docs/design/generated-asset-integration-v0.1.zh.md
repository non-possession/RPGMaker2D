...
已完成 OBJ02-B/OBJ02-C/OBJ06 的本地确定性精修：

- 新增 `scripts/tools/refine_text_readability_assets.py`，用于复现这轮文字降噪处理。
- OBJ02-B 黑板拍照状态：保留粉笔痕迹、儿童涂鸦和“拍照后信息更多”的密度，但把清晰人名、日期、句子压成破碎粉笔线。
- OBJ02-C 黑板最终状态：保留暖光和最终显影感，但避免出现可逐字阅读的大字。
- OBJ06 档案柜/花名册/旧照片：保留“旧记录”和“旧照片”的形状信息，降低花名册和照片细节清晰度，避免被误读为需要辨认姓名或照片内容。

校准结果：

- 资产尺寸和路径未改变，现有 `Sprite2D` 坐标、交互区、完成纸签无需重新校准。
- 这轮是运行时资产直接 v2 替换；v1 状态保留在 Git 历史中。

## 第三批节奏 v1 收束记录

已完成一轮交互节奏收束：

- 对话框打开时增加轻微淡入，文本逐行轻淡入，避免文本“硬切”。
- 每行对话按说话者和文本长度增加最小阅读停顿；`E / Enter 继续` 提示会在停顿后出现，减少玩家误触跳过情绪句。
- 拍照事件的顺序调整为：交互触发 -> 快门/取景框反馈 -> 显影 -> 对话文本。这样 A5/A10/A12 的“拍照记录”更像一个动作，而不是普通调查。
- 非拍照显影事件保留更短的前置停顿，让 A7/A8/A11 的情绪浮起有一点呼吸，但不拖慢工作流。
